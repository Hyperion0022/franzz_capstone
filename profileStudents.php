...
$stmt = $conn->prepare("SELECT first_name, last_name, email, profile_picture, cover_photo, bio, role FROM students WHERE id = ?");
$stmt->execute([$user_id]);
$teacher = $stmt->fetch();

// Fallback values
$profile_picture = isset($teacher['profile_picture']) && !empty($teacher['profile_picture']) ? $teacher['profile_picture'] : 'PROFILE.webp';
$cover_photo = isset($teacher['cover_photo']) && !empty($teacher['cover_photo']) ? $teacher['cover_photo'] : 'cover.jpg';
$bio = !empty($teacher['bio']) ? $teacher['bio'] : 'Look always seems imposible until its done | Lifelong Learner ✨';
?>
<!DOCTYPE html>
<html lang="en">
<head>
